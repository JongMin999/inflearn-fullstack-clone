...
export default function EditCourseSidebar() {
  ...
  return (
    <Card className="w-60 p-4 md:p-6 flex flex-col gap-6 md:gap-8 bg-white border-none rounded-lg shadow-none flex-shrink-0">
      {steps.map((section) => (
        <div key={section.section}>
          <div className="font-bold text-sm md:text-base lg:text-lg mb-3 md:mb-4 whitespace-nowrap">{section.section}</div>
          <ol className="flex flex-col gap-2">
            {section.items.map((item, idx) => {
              ...
              return (
                <li key={item.key} className="flex items-start gap-2 md:gap-3">
                  <Link
                    href={item.href(id)}
                    className={`flex items-center gap-2 md:gap-3 py-2 px-2 md:px-3 rounded-lg w-full transition-colors flex-nowrap
                      ${
                        isActive
                    ...
                    `}
                  >
                    <span className="flex items-center justify-center w-6 h-6 md:w-7 md:h-7 flex-shrink-0">
                      <span
                        className={`w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-full border text-xs md:text-sm
                            ${
                              isActive
